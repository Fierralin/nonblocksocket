...
//              state->buffer[state->buffer_used++] = buf[i]; //接收什么发送什么，不经过转换，测试用
    ...
    //判断最后一次接收的字节数
    if (result == 0) free_fd_state(state);
    else if (result < 0) {
        if (errno == EAGAIN) return; // XXXX use evutil macro
        perror("recv");
        free_fd_state(state);
    }
}
...
void do_write(evutil_socket_t fd, short events, void *arg) {
    ...
    while (state->n_written < state->write_upto) {
        ...
        if (result < 0) {
            if (errno == EAGAIN) return; // XXX use evutil macro
            free_fd_state(state);
            return;
        }
        assert(result != 0);
...
void do_accept(evutil_socket_t listener, short event, void *arg) {
  ...
  if (fd < 0) perror("accept"); // XXXX eagain??
  else if (fd > FD_SETSIZE) close(fd); // XXX replace all closes with EVUTIL_CLOSESOCKET */
  else {
