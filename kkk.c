...
main() {
  ...
  flags = fcntl(last_fd, F_GETFL, 0);
  fcntl(last_fd, F_SETFL, flags | O_NONBLOCK); /* Change the socket into non-blocking state */
  flags = fcntl(new_fd, F_GETFL, 0);
  fcntl(new_fd, F_SETFL, flags | O_NONBLOCK); /* Change the socket into non-blocking state  */
